define(['modules/settings', 'modules/templates', 'lib/underscore', 'jquery'], function(w,templates) {
  ...
  function exportVocab() {
    ...
    $("._tr__export").attr('href', "data:application/octet-stream;charset=utf-8," + encodeString(formatted_vocab));
  }
  ...
  // Translation
  $(document).on('click', '._tr_translate', function(e) {
    ...
    translateText(text, source_language, target_language).done(function(result) {
      ...
      $("._tr_body").html(markup);
      $("._tr_list_toggle").attr("data-word", text);
      $("._tr_list_toggle").show();
    }).fail(function(jqXHR) {
  ...
  // exporting
  $(document).on('click', '._tr_list_toggle', function(e) {
    toggleItemExport($("._tr__list_toggle").attr("data-word"));
    if ($(this).parents("._tr__definition") !== []) {
      killTraductor();
    }
  });

  $(document).on('click', '._tr_export', function(e){
    exportVocab();
  });

  // various ways to quit
  $(document).on('click', '._tr_quit', function(e) {
    killTraductor();
  });

  $(document).on('mousedown', 'body', function(e){
    if ($(e.target).parents("._tr").length < 1) {
      killTraductor();
    }
  });
